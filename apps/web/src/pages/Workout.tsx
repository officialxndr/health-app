    ...
    <div className="bg-surface rounded-2xl p-4">
      ...
      <div className="flex flex-wrap gap-1.5 mt-2">
        ...
        {template.exercises.length === 0 && (
          <span className="text-xs text-muted">No exercises</span>
        )}
      </div>
    </div>
  )
...
function SessionCard({
  ...
  return (
    <button onClick={onTap} className="w-full text-left bg-surface rounded-2xl p-4">
      ...
      <div className="flex gap-4 text-sm text-muted">
        ...
        {prs > 0 && <span className="text-warning">🏆 {prs} PR{prs > 1 ? 's' : ''}</span>}
      </div>
    </button>
  )
}
...
export function Workout() {
  ...
  const [tab, setTab] = useState<'templates' | 'history' | 'exercises'>('templates')
    ...
    <div className="flex-1 min-h-0 flex flex-col">
      ...
      <div className="bg-surface border-b border-border px-4 py-3 safe-top">
        ...
        <div className="flex bg-surfaceHigh rounded-xl p-1 gap-1">
          {(['templates', 'history', 'exercises'] as const).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
          ...
          {tab === 'history' && (
            <>
              ...
              {sessions.length === 0 ? (
                ...
                sessions.map(s => (
                  <SessionCard key={s.id} session={s} onTap={() => setSelectedSession(s)} />
                ))
              )}
            </>
          )}
