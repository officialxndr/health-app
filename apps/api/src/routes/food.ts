...
const foodRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // Search food items: local DB + parallel USDA (generic) + OFF (branded)
  fastify.get<{ Querystring: { q: string } }>(
    ...
    async (request) => {
      ...
      // Fire USDA (SR Legacy + Foundation = generic/unbranded) and OFF in parallel
      const [usdaResult, offResult] = await Promise.allSettled([
        ...
        axios.get(`${OFF_BASE}/cgi/search.pl`, {
          params: {
            search_terms: q,
            search_simple: 1,
            action: 'process',
      ...
      // Add USDA generic results first (they're the most useful for generic queries)
      const usdaItems: typeof local = []
      if (usdaResult.status === 'fulfilled') {
        const foods: any[] = usdaResult.value.data?.foods ?? []
        for (const f of foods) {
          const itemData = usdaToItem(f)
          if (!itemData) continue
          // Skip if we already have this from local cache
          if (local.some((l) => l.name.toLowerCase() === itemData.name.toLowerCase())) continue
          try {
            const item = await prisma.foodItem.create({ data: itemData })
            usdaItems.push(item)
          } catch { /* may already exist */ }
        }
      }

      // Add OFF branded results after (filtered for quality)
      const offItems: typeof local = []
      if (offResult.status === 'fulfilled') {
        ...
        for (const p of products) {
          ...
          const createData = {
            ...
            const item = barcode
              ? await prisma.foodItem.upsert({ where: { barcode }, update: {}, create: createData })
              : await prisma.foodItem.create({ data: createData })
            if (!local.some((l) => l.id === item.id)) offItems.push(item)
  ...
  // Dates within a month that have at least one food log entry (for calendar dots)
  fastify.get<{ Querystring: { month: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!month || !/^\d{4}-\d{2}$/.test(month)) {
        return reply.status(400).send({ error: 'month param required (YYYY-MM)' })
      }
  ...
  fastify.get(
    ...
    async (request) => {
      ...
      return favorites.map((f) => f.foodItem)
    }
  )

  // Add a food item to favorites
  fastify.post<{ Body: { foodItemId: string } }>(
    '/favorites',
    { onRequest: [fastify.authenticate] },
    async (request, reply) => {
      const userId = request.user.userId
      const { foodItemId } = request.body
      if (!foodItemId) return reply.status(400).send({ error: 'foodItemId required' })
      await prisma.favoriteFoodItem.upsert({
  ...
  fastify.delete<{ Params: { foodItemId: string } }>(
    '/favorites/:foodItemId',
    { onRequest: [fastify.authenticate] },
    async (request, reply) => {
      const userId = request.user.userId
      const { foodItemId } = request.params
      await prisma.favoriteFoodItem.deleteMany({ where: { userId, foodItemId } })
